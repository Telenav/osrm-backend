...
// StationConnectivityQuerier implements interface of place.TopoQuerier based on pre-build connectivity data.
type StationConnectivityQuerier struct {
	stationLocationQuerier   place.LocationQuerier
	stationConnectivity      *topograph.ConnectivityMap
	reachableStationsByStart []*entity.RankedPlaceInfo
	reachableStationToEnd    map[entity.PlaceID]*entity.RankedPlaceInfo
	startLocation            *nav.Location
	endLocation              *nav.Location
}

// New creates place.TopoQuerier interface which is implemented by StationConnectivityQuerier
...
func (querier *StationConnectivityQuerier) connectStartIntoStationGraph(stationFinder place.Finder, stationRanker place.Ranker,
...
func (querier *StationConnectivityQuerier) connectEndIntoStationGraph(stationFinder place.Finder, stationRanker place.Ranker,
	...
	reachableStationToEnd := make(map[entity.PlaceID]*entity.RankedPlaceInfo)
	for _, rankedPointInfo := range rankedPoints {
		reachableStationToEnd[rankedPointInfo.ID] = &entity.RankedPlaceInfo{
...
func (querier *StationConnectivityQuerier) NearByStationQuery(placeID entity.PlaceID) []*entity.RankedPlaceInfo {
	...
	if connectivityResults, ok := querier.stationConnectivity.QueryConnectivity(placeID); ok {
		if !querier.isStationConnectsToEnd(placeID) {
			...
			results := make([]*entity.RankedPlaceInfo, 0, size)
		...
		if querier.isStationConnectsToEnd(placeID) {
			results := make([]*entity.RankedPlaceInfo, 0, 1)
			return querier.connectEndIntoGraph(placeID, results)
		}
	}
...
func (querier *StationConnectivityQuerier) GetLocation(placeID entity.PlaceID) *nav.Location {
	switch placeID {
...
func (querier *StationConnectivityQuerier) isStationConnectsToEnd(placeID entity.PlaceID) bool {
	_, ok := querier.reachableStationToEnd[placeID]
	return ok
}

func (querier *StationConnectivityQuerier) connectEndIntoGraph(placeID entity.PlaceID, results []*entity.RankedPlaceInfo) []*entity.RankedPlaceInfo {
...
func appendIntoSortedSlice(item *entity.RankedPlaceInfo, results []*entity.RankedPlaceInfo) []*entity.RankedPlaceInfo {
